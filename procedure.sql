...
BEGIN
    ...
    IF @newParentId IS NOT NULL AND EXISTS (SELECT 1
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF (EXISTS(select top 1 id from product where category_id = @categoryId))
            BEGIN
        ...
        IF @parentId IS NULL
            BEGIN
                UPDATE category
                SET root = 1
                WHERE id IN (SELECT descendant_id FROM category_closure WHERE ancestor_id = @categoryId AND depth = 1);
            END
        ELSE
            BEGIN
                INSERT INTO category_closure (ancestor_id, descendant_id, depth)
